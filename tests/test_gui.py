...
from transitio.gui import create_app  # noqa: E402
...
def test_cli_requires_existing_feed(tmp_path, capsys):
    from transitio._cli import main

    with pytest.raises(SystemExit):
        main(["edit", str(tmp_path / "absent.zip")])
...
def test_cli_refuses_non_loopback_host(tmp_path):
    ...
    with pytest.raises(SystemExit):
        main(["edit", str(feed), "--host", "0.0.0.0"])
